...
public class PassengerDetails {
	...
	// Method to generate random PNR numbers
	public int generateRandomPNR() {
		// Generate random 8-digit number (between 10000000 and 99999999)
		return 10000000 + random.nextInt(90000000);
	}

	public void processPassengerDetails(int nop) {
		// Assuming nop is the number of passengers
		for (int i = 1; i <= nop; i++) {
			pnr1[i] = generateRandomPNR();
		}
	}
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					window. processPassengerDetails(nop);
...
//					int PNR = rs.getInt("PNR");
					...
					for(int i = 0, j = 0;i < nop;i++)
					{
						...
						case 0 : name = textField.getText();
								 ...
								 query = "insert into Passenger (pnr,User_name,Age,p_name,Gender,Source,Destination,Train_no) values ('"+pnr1[j]+"," + u_id + "'," + age + ",'" + name +"','"+gender+"','"+src+"','"+dest+"',"+tno+")";
								 st.executeUpdate(query);
								 pnr[i] = pnr1[j] ;
								 p_name[i] = name;
								 p_age[i] = age;
